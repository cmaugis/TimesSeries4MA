...
$$ où $(\varepsilon_t)_{t\in \Z}$ est un $\text{bbF}(0,\sigma^2)$ tel que $\E[\varepsilon_t^4]<+\infty$ et où la suite des coefficients $(\psi_j)_{j\in \Z}$ est sommable et de somme non nulle $$
...
Le **périodogramme** associé à $(X_1,\ldots,X_n)$ est défini par $$
...
A FAIRE - Peut-être écrire la proposition précédente par rapport à $\hat f_X(.)$
:::
...
::: {#prp-p3 .proposition}
Le sous-espace vectoriel fermé engendré par une famille **finie** $(X_t)_{t\in I}$, où $I \subset \Z$, est l'ensemble de toutes les combinaisons linéaires, i.e. l'ensemble des v.a. $Y$ de la forme $Y=\sum_{i\in I}\alpha_i X_i$.
:::

::: {#def-proj .definition}
Soit l'espace vectoriel fermé engendré par une famille finie $(X_t)_{t\in I}$ de v.a. de $L^2 (\Omega,\mathcal A,P)$, où $I\subset\Z$ fini : $\mathcal H=\overline{\text{Vect}}(X_t,t\in I)$.
...
\end{eqnarray*} où $\mathcal M(X_1,\ldots,X_n)=\left\{g(X_1,\ldots,X_n); g \text{ fonction borélienne de } \R^n \text{ vers } \R\right\}$ et $\sigma(X_1,\ldots,X_n)$ est la tribu engendrée par les v.a. $X_1,\ldots,X_n$.
...
-   Par définition de $\H^{t-1}_{-\infty}$ et de $\hat X_t$, on a $$
    ...
    $$ donc $\E[X_t]=\E[\hat X_t]$, ce qui prouve que le processus des innovations est centré.
...
::: {#prp-ELO .proposition}
Soit $(X_t)_{t\in \Z}$ un série temporelle stationnaire de moyenne $\mu_X$ et d'ACVF $\gamma_X(\cdot)$. La **prévision linéaire optimale** de $X_{n+h}$, pour $h\in \N^*$, ayant observé le passé $X_1,\ldots,X_n$, est $$
...
$$ Or d'après le @lem-lemma1, \begin{eqnarray*}
...
<br> On rejette $\mathcal H_0$ au risque $\alpha$ si $Q_k$ ou $Q_k^*$ est supérieure au $(1-\alpha)$-quantile de la loi du $\chi^2(k)$.

Zone de rejet : $$
\mathcal R_\alpha^{(k)}=\left\{Q_k^* > q_{1-\alpha,k}\right\}
\textrm{ où }
\mathcal R_\alpha^{(k)}=\left\{Q_k > q_{1-\alpha,k}\right\}
$$ avec $q_{1-\alpha,k}$ est le $(1-\alpha)$-quantile de la loi $\chi^2(k)$ car $$
