...
<br> La @fig-exmarchealea montre des exemples de trajectoires de marches aléatoires.
...
Si le processus $(X_t)_{t\in \mathbb Z}$ est un bruit blanc fort dans $L^2$ alors il est faiblement stationnaire puisque : $$
...
::: {#def-ACVF .definition}
Soit $(X_t)_{t\in\Z}$ un processus faiblement stationnaire.
...
::: {#def-ACF .definition}
Soit $(X_t)_{t\in\Z}$ un proecssus faiblement stationnaire.
...
<br> Soit $(X_t)_{t\in \Z}$ une série temporelle MA(1) définie par $$
...
\end{eqnarray*}
```
<br> $\Leftarrow$) Soit $K:\Z\rightarrow\R$ paire et demi-définie positive. On veut montrer qu'il existe un processus stationnaire tel que $\gamma_X(.)=K(.)$. <br>
...
$$ où $\mathbf{u}=(u_1,\ldots,u_n)'\in\R^n$ et la matrice $\mathbb{K}=(K(t_i - t_j))_{i,j=1,\ldots,n}$. <br> Comme $K$ est semi-définie positive, $\mathbb{K}$ est une matrice semi-définie positive et donc $\Phi_{\mathbf{t}}(.)$ est la fonction caractéristique d'un vecteur gaussien $\mathcal{N}(0,\mathbb K)$.
...
$$ où $\mathbf{t}_{-i} = (t_1,\ldots,t_{i-1},t_{i+1},\ldots,t_n)'$ et $\mathbf{u}_{-i} = (u_1,\ldots, u_{i-1}, u_{i+1},\ldots,u_n)'$. On conclut la preuve en utilisant le théorème d'existence de Kolmogorov : <br> $\{F_{\mathbf{t}}(.),\ \mathbf{t}\in \mathcal{T}\}$ sont les fonctions de distribution d'un processus stochastique si et seulement si pour tout $n$, $\mathbf{t}=(t_1,\ldots,t_n)\in\mathcal{T}$, $\mathbf{x}\in\R^n$ et $1\leq k \leq n$, $$
...
<br> La densité spectrale d'un processus stationnaire $(X_t)_{t\in \Z}$ de fonction d'autocovariance $\gamma_X(\cdot)$ existe dès que l'on a : $$
