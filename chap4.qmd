...
On a déjà utilisé ce type d'opérateur pour définir les moyennes mobiles (voir chapitre @sec-chap1). Nous avons également considéré la notion de série en $B$ quand nous avons introduit la notion de filtrage linéaire (voir @sec-chap3). Rappelons que nous avons vu que si $(X_t)_{t\in \Z}$ est un processus stationnaire, alors le processus $(Y_t)_{t\in \Z}$ défini par $$Y_t=\sum_{i\in \Z}a_iX_{t-i}$$ est également stationnaire, sous la condition de la convergence normale de la série des coefficients réels, i.e. sous l'hypothèse $\sum_{i\in \Z}|a_i|<+\infty$.

::: {#def-serieB .definition}
Soit $(a_i)_{i\in \Z}$ une famille absolument sommable de réels ($\sum_{i\in \Z}|a_i|<+\infty$). <br> On appelle [**série en** $B$]{style="color:blue;"} de coefficients $(a_i)_{i\in \Z}$ l'opérateur $$
...
\left(\lambda P(B)+Q(B)\right)X_t&=&\lambda
P(B)X_t+Q(B)X_t=\sum_{i\in \Z}\lambda a_iX_{t-i}+\sum_{i\in \Z}b_iX_{t-i}\\
&=&\lim_{n\to +\infty}\sum_{i=-n}^{i=+n}\lambda a_iX_{t-i}+\lim_{n\to
+\infty}\sum_{i=-n}^{i=+n}b_iX_{t-i}\\ &=&\lim_{n\to
+\infty}\sum_{i=-n}^{i=+n}\left(\lambda a_iX_{t-i}+b_iX_{t-i}\right)=\sum_{i\in
\Z}(\lambda a_i+b_i)X_{t-i}, 
\end{eqnarray*} où la dernière égalité est justifiée par l'absolue convergence de la série $\sum(\lambda a_i+b_i)$ ga rantie par l'inégalité $$ 
\sum_{i\in \Z} |\lambda a_i+b_i|\leq \lambda\sum_{i\in \Z}|
a_i|+\sum_{i\in \Z}|b_i|<+\infty. 
$$ On a bien alors $$
\lambda\sum_{i\in \Z}
a_iB^i+\sum_{i\in \Z} b_iB^i=\sum_{i\in \Z}(\lambda a_i+b_i)B^i. $$ Considérons maintenant la composition des deux séries $P(B)$ et $Q(B)$. On a $$
...
-   Dans l'étude des modèles ARMA, on va avoir besoin de savoir quand et comment on peut inverser des polynômes en $B$
-   Commençons par un polynôme de degré 1 en $B$ : $I-\lambda B$.
-   Cet opérateur associe à tout processus stationnaire $(X_t)_{t\in \Z}$ un nouveau processus stationnaire $(Y_t)_{t\in \Z}$ tel que $$ 
    Y_t=(I-\lambda
    B)X_t=X_t-\lambda X_{t-1}. 
    $$
-   Le problème de l'inversion du polynôme $I-\lambda B$ revient, une fois donné un processus stationnaire $(Y_t)_{t\in \Z}$, à déterminer s'il existe un processus stationnaire $(X_t)_{t\in \Z}$ vérifiant : $$ 
    Y_t=(I-\lambda B)X_t=X_t-\lambda X_{t-1} 
    $$ et de donner son expression en fonction du processus $(Y_t)_{t\in \Z}$.

On va constater que ce polynôme $I-\lambda B$ est inversible seulement si $|\lambda|\neq 1$ et on obtiendra dans ce cas l'expression de son inverse.
...
-   La série en $B$ de coefficients $(a_i)_{i\in \Z}$ est donc bien définie et on peut écrire $$
    (I-\lambda B)\sum_{i\in \Z}a_iB^i=(I-\lambda B)\sum_{i\in
    \N}\lambda^iB^i=\sum_{i\in \N}\lambda^iB^i-\sum_{i\in \N}\lambda^{i+1}B^{i+1}=I.
    $$
...
-   Mais $(X_t)_{t\in\Z}$ avec $X_t=\sum_{i=0}^{+\infty}\lambda^iY_{t-i}$ n'est pas l'unique solution : soit $(X^*_t)_{t\in \Z}$ tel que $(I-\lambda B)X_t^*=0$. Le processus $(\tilde X_t)_{t\in \Z}$ défini par $\tilde X_t=X_t+X_t^*$ est tel que $$ 
    Y_t=(I-\lambda B)\tilde X_t. 
    $$

-   Un processus $(X^*_t)_{t\in \Z}$ vérifiant $X_t^*-\lambda X_{t-1}^*=0$ est de la forme $X^*_t=\lambda^t A$, pour tout $t\in\Z$, où $A$ est une v.a.r. de $L^2$.

-   Ainsi, étant donné un processus $(Y_t)_{t\in \Z}$, les solutions de l'équation $$ 
    Y_t=(I-\lambda B)X_t,\ \forall t\in\Z 
    $$ sont de la forme $$
...
-   Etant donné le processus $(Y_t)_{t\in \Z}$, **la solution stationnaire** de l'équation $$ 
    Y_t=(I-\lambda B)X_t, 
    $$ est le processus $(X_t)_{t\in \Z}$ défini par $$ 
    X_t=\sum_{i=0}^{+\infty}\lambda^iY_{t-i}. 
    $$

::: proposiiton
L'opérateur $(I-\lambda B)$ est donc inversible dans la classe des processus stationnaires d'inverse $\underset{i=0}{\stackrel{+\infty}{\sum}}\lambda^iB^i$ quand $|\lambda|<1$.
:::
...
-   On peut écrire $$ 
    ...
    $$
-   Comme $|\frac 1 \lambda|<1$, par le même raisonnement que précédent, le polynôme $\left(I-\frac{1}{\lambda}F\right)$ est inversible dans la classe des processus stationnaires d'inverse $\sum_{i=0}^{+\infty}\lambda^{-i}F^i$.
-   Le processus $-\lambda B$ est inversible d'inverse $(-1/\lambda)F$.

::: proposition
Quand $|\lambda|>1$, le processus $I-\lambda B$ est inversible dans la classe des processus stationnaires d'inverse $$
\left(\frac{-1}{\lambda}F\right)\sum_{i=0}^{+\infty}\frac{1}{\lambda^i}F^i=-\sum_{i=0}^{+\infty}\frac{1}{\lambda^{i+1}}F^{i+1}=-\sum_{j=1}^{+\infty}\frac{1}{\lambda^j}F^j
$$
:::

-   Le processus $(Y_t)_{t\in \Z}$ étant donné, l'unique solution stationnaire de l'équation $$ 
    Y_t=(I-\lambda B)X_t, 
    $$ est donc $(X_t)_{t\in \Z}$ définie par $$ 
    X_t=-\sum_{j=1}^{+\infty}\frac{1}{\lambda^j}Y_{t+j}. 
    $$
-   Remarque : cette fois-ci, l'expression de la solution $(X_t)_{t\in \Z}$ est fonction du futur du processus $(Y_t)_{t\in \Z}$ et non en fonction de son passé comme dans le cas $|\lambda|<1$.
...
-   Soit $(X_t)_{t\in \Z}$ le processus constant $X_t=m,\ \forall t\in\Z$. On a alors $$ 
    (I- B)X_t= m-m=0, 
    $$ ce qui prouve que l'opérateur n'est pas injectif et donc pas bijectif.
...
X_t-X_{t-1}=m,\ \forall t\in\Z 
$$ On arrive alors à une contradiction car $\E [X_t-X_{t-1}]=0\neq m$.

### Inverse d'un polynôme en $B$

-   Considérons comme opérateur un polynôme en $B$ de la forme $$
    \Phi(B)=I-\varphi_1B-\varphi_2B^2-\cdots-\varphi_pB^p. 
    $$
-   Question : $\Phi(B)$ est-il inversible ? Se donnant un processus $(Y_t)_{t\in \Z}$, existe-t-il un unique processus stationnaire $(X_t)_{t\in \Z}$ tel que $$ 
    \Phi(B)X_t=Y_t,\ \forall t\in\Z. 
    $$
-   Dans les complexes, ce polynôme a $p$ racines non nécessairement distinctes. Comme il est non constant, ce polynome est scindé. On peut donc alors utiliser les résultats du polynôme $I-\lambda B$.
...
\left(1-\varphi_1z-\varphi_2z^2-\cdots-\varphi_pz^p\right)  \sum_{i\in\N}\psi_iz^i=1 
...
On dit qu'un processus stationnaire $(X_t)_{t\in \Z}$ admet une représentation **auto-régressive d'ordre** $p$ (noté AR(p)) s'il vérifie l'équation récurrente : $$
...
-   Illustration dans le cas AR(1) : Soit un processus AR(1) vérifiant $$
...
-   On a vu précédemment que dans ce cas, il n'existe pas de processus stationnaire $(X_t)_{t\in \Z}$ vérifiant l'équation $$ 
    (I-\varphi B)X_t=\varepsilon_t. 
    $$
-   Il n'existe donc pas de processus AR(1) de paramètre 1 ou -1.
...
-   Cette représentation est canonique :

    -   on déduit que $X_t\in\mathcal H_{-\infty}^t(\varepsilon)$ et donc $\mathcal H_{-\infty}^t(X)\subset \mathcal H_{-\infty}^t(\varepsilon)$ pour tout $t$ dans $\Z$
    -   par propriété des bruits blancs $\varepsilon_t\perp \mathcal H_{-\infty}^{t-1}(\varepsilon)$

$\hspace{2cm}\Longrightarrow \varepsilon_t\perp\mathcal H_{-\infty}^{t-1}(X)$
...
-   Le processus des innovations : $$ 
    ...
    $$ C'est donc le bruit blanc $(\varepsilon_t)_{t\in \Z}$ dans la représentation AR(1).

On verra que ce résultat reste valable pour tout modèle AR de représentation canonique.

#### Cas $|\varphi|>1$

-   Le polynôme $I-\varphi B$ est inversible d'inverse $$ 
    (I-\varphi B)^{-1}=-\sum_{i=1}^{+\infty}\frac{1}{\varphi^i}F^i \textrm{  d'où  }
    X_t=-\sum_{i=1}^{+\infty}\frac{1}{\varphi^i}\varepsilon_{t+i}. 
    $$

-   Cette représentation du processus AR(1) **n'est pas canonique** :
...
-   Processus des innovations : \begin{eqnarray*} 
    X_{t+1}-\hat X_t(1)&=&\varphi
    X_t+\varepsilon_{t+1}-\varphi X_t-\EL(\varepsilon_{t+1}|\mathcal
    H_{-\infty}^t(X))\\ &=&\varepsilon_{t+1}-\EL(\varepsilon_{t+1}|\mathcal
    H_{-\infty}^t(X)) 
    \end{eqnarray*} donc non confondus avec le bruit blanc utilisé dans la représentation (non canonique) du processus AR(1).

#### Cas $|\varphi|>1$ : vers une représentation canonique

-   On va ici montrer que le processus AR(1) pour $|\varphi|>1$ possède un autre représentation, qui est canonique.

-   On utilise la formule reliant les densités spectrales de deux processus stationnaires dont l'un est le filtrage linéaire d'un autre :

Partant de $(I-\varphi B)X_t=\varepsilon_t$, on a $$ 
...
-   Soit le processus $(\eta_t)_{t\in \Z}$ défini par $\eta_t=X_t-\frac{1}{\varphi}X_{t-1},\ \forall t \in \Z$. \begin{eqnarray*}
...
-   Nouvelle représentation AR(1) pour le processus $(X_t)_{t\in \Z}$ : $$
    (I-\frac{1}{\varphi} B)X_t=\eta_t \textrm{ où } (\eta_t)_{t\in \Z}\sim
    \text{WN}(0,\sigma^2/\varphi^2) 
    $$ et la représentation est canonique car $|1/\varphi|<1$.

On se retrouve aussi que le processus $(\eta_t)_{t\in \Z}$ correspond au processus des innovations.

Cette petite étude du cas particulier du processus AR(1) nous a donc permis de voir que la représentation AR(1) n'est pas unique et qu'il suffit de prendre celle avec le coefficient $\varphi$ ou $1/\varphi$ inférieur à 1 pour "tomber" sur la représentation canonique. Ce résultat se généralise aux processus AR d'ordre quelconque.

#### Propriétés

Avant d'étudier plus en détail les propriétés des processus AR, notons, et c'est la moindre des choses, que de tels processus existent ! La proposition suivante généralise les résultats vus sur la surjectivité et l'inversibilité du polynôme en $I-\lambda B$ et décrit le processus AR en fonction du bruit blanc.
...
-   Il existe une infinité de processus du second ordre $(X_t)_{t\in \Z}$ vérifiant : $$
    ...
    $$
-   Si les racines du polynôme $\Phi(B)$ sont \textbf{toutes de module différent de 1}, il existe une **unique solution stationnaire** $(X_t)_{t\in \Z}$. Elle s'écrit alors sous la forme d'un filtrage linéaire du bruit blanc $(\varepsilon_t)_{t\in \Z}$ : $$ 
...
Pour s'en convaincre, il suffit de factoriser le polynôme $\Phi(B)$ de degré $p$ en fonction de ses $p$ racines et d'appliquer $p$ fois le résultat d'inversibilité vu pour le polynôme $I-\lambda B$. C'est ce que nous avons décrit dans le paragraphe "Inverse_poly_B" de ce même chapitre.
:::
...
On montre seulement le dernier point. Le second est en effet évident et le premier point s'obtiendrait de manière tout à fait identique à ce qui suit en faisant un changement de racines qui ne retient pas uniquement celles à l'extérieur du disque unité. La démonstration est assez proche de ce que l'on a fait pour le processus AR(1) et utilise donc les densités spectrales. De l'équation $$
\Phi(B)X_t=\varepsilon_t, \text{ pour tout }t\in \Z, 
$$ on tire $$
...
\right|^2. 
$$ En supposant toutes les racines de $\Phi(z)$ réelles, on peut refaire le même genre de raisonnement que celui fait pour le AR(1) et obtenir \begin{eqnarray*} f_\eta(\omega)&=&\frac{\left| \tilde\Phi(e^{-i\omega})
...
\psi_i\eta_{t-i}. $$ On en déduit l'inclusion inverse : $\H_{-\infty}^t(X)\subset \H_{-\infty}^t(\eta)$ et donc l'égalité. La représentation canonique est alors bien acquise puisque, en tant que bruit blanc, le processus $(\eta_t)_{t\in \Z}$ est évidemment orthongonal à son passé, donc ici également orthogonal au passé de $(X_t)_{t\in \Z}$. \hfill $\Box$
:::
...
-   Variance du processus $(X_t)_{t\in \Z}$ :\
    ...
    canonique)}. 
    \end{eqnarray*} où la dernière égalité est obtenue par orthogonalité du bruit blanc par rapport à l'histoire du processus AR($p$) dans sa représentation canonique. On a alors Donc $$
    \gamma_X(0)=\frac{\sigma^2}{1-\sum_{i=1}^p\varphi_i\rho_X(i)}. 
    $$

On peut faire de même pour les autocovariances : pour $h>0$, \begin{eqnarray*}
...
\end{eqnarray*} D'où les autocorrélations satisfont l'équation linéaire récurrente d'ordre $p$ $$
...
**Autocorrélations simples d'un AR(p)** Les autocorrélations simples d'un processus AR(p) décroissent, de manière exponentielle ou sinusoïdale amortie, vers 0.
:::

::: {.callout-warning collapse="true" icon="false"}
### Proof

L'équation (\ref{Recurrence_rho_AR}) vérifiée par les autocorrélations d'un AR($p$) est une équation de récurrence linéaire de polynôme caractéristique $$ r^p-\varphi_1r^{p-1}-\ldots
-\varphi_{p-1}r-\varphi_p=r^p\Phi\left(\frac{1}{r}\right). $$ Les racines (évidemment non nulles, puisque par définition $\varphi_p$ est non nul) de ce polynôme sont les inverses de celles du polynôme $\Phi(z)$ définissant l'AR($p$). D'après ce que l'on sait sur les solutions d'un équation linéaire récurrente, la solution de l'équation (\ref{Recurrence_rho_AR}) dans $\mathbb C$ est donc une combinaison linéaire de solutions de la forme $$
(a_{i,0}+a_{i,1}h+\cdots+a_{i,k-1}h^{k-1})\lambda_i^h, $$ où $k$ est l'ordre de multiplicité de la racine $z_i=1/\lambda_i$ de $\Phi(z)$. Or, puisqu'on considère la représentation canonique, les racines $z_i$ de $\Phi(z)$ sont de module à l'extérieur du disque unité. Ainsi, si toutes les solutions sont réelles alors les autocorrélations décroissent effectivement de manière exponentielle vers 0. Si une des racines $z_i=1/\lambda_i$ de $\Phi(z)$ n'est pas réelle, on sait alors que son conjugué l'est aussi et que les solutions réelles correspondant à ces deux racines conjuguées s'écrivent de la forme $$
|\lambda_i|^h\left[(a_{i,0}+a_{i,1}h+\cdots+a_{i,k-1}h^{k-1})\cos(\omega_i
h)+(b_{i,0}+b_{i,1}h+\cdots+b_{i,k-1}h^{k-1})\sin(\omega_i h)\right], $$ où $\omega_i$ est le module de $\lambda_i$. Cette partie de Elles sont donc bien simusoïdales amorties.
:::
...
$X_t = 0.7 X_{t-1}-0.1 X_{t-2}+\varepsilon_t$
...
AR(2) - racines $2\pm i 2\sqrt{3}/3$

$X_t = \frac{3}{4} X_{t-1} - \frac{3}{16} X_{t-2}+\varepsilon_t$
...
&=&\varphi_1X_{t-1}+\varphi_2X_{t-2}+\cdots+\varphi_pX_{t-p}+0\times
X_{t-p-1}+\cdots+ 0\times X_{t-k} 
...
#| echo: false
#| fig-height: 11
ggpacf(Xt1)+ggtitle(TeX(r'($X_t = 0.7 X_{t-1}-0.1 X_{t-2}+\epsilon_t$)'))+
...
ggpacf(Xt3)+ggtitle(TeX(r'($X_t = 0.7 X_{t-1}-0.1 X_{t-2}+0.2 X_{t-3}+\epsilon_t$)'))+
theme(plot.title = element_text(size = 40))
```
:::
...
::: {#def-MAq .definition}
On dit qu'un processus stationnaire $(X_t)_{t\in \Z}$ admet une représentation en **Moyenne Mobile d'ordre** $q$ (noté MA(q)) s'il vérifie l'équation $$
...
Rem : Contrairement à un AR, un MA est entièrement spécifié. A bruit blanc et paramètres $\theta_1,\ldots,\theta_q$ fixés, il ne correspond qu'un seul processus MA($q$) qui est le filtrage linéaire de $(\varepsilon_t)_{t\in \Z}$ par le filtre $$
...
$$ - Le processus des innovations correspond alors au bruit blanc $(\varepsilon_t)_{t\in \Z}$ de sa représentation canonique. - Le processus $(X_t)_{t\in \Z}$ possède une représentation AR($\infty$) : $$
...
On va d'abord établir la représentation AR($\infty$). Comme mes racines du polynôme $\Theta(z)$ sont, par hypothèse que la représentation est canonique, toutes situées à l'extérieur du disque unité, l'opérateur $\Theta(B)$ est inversible d'inverse $$
...
$$ Ces écritures MA($q$) et AR($\infty$) du processus $(X_t)_{t\in \Z}$ montrent qu'il est régulier, i.e. $$ 
\H_{-\infty}^t(X)= \H_{-\infty}^t(\varepsilon). 
$$ On peut ainsi écrire \begin{eqnarray*}
\EL(X_t|\H_{-\infty}^{t-1}(X))&=&\EL(X_t|\H_{-\infty}^{t-1}(\varepsilon))=\EL(
...
\end{eqnarray*}
```
-   Pour la fonction d'autocovariance, on a $$
      \gamma_X(h)=\C(X_t,X_{t+h})=\C\left(\varepsilon_t +
      \sum_{j=1}^q\theta_j\varepsilon_{t-j},\varepsilon_{t+h} +
      \sum_{i=1}^q\theta_i\varepsilon_{t+h-i}\right). 
    $$

-   On a immédiatement que $\gamma_X(h)=0$ quand $h>q$.

-   Pour $h=1$ \begin{eqnarray*}
    \C(X_t,X_{t+1})&=&\E\left[(\varepsilon_t +
...
-   Pour $h=2$ : \begin{eqnarray*}
    \C(X_t,X_{t+2})&=&\E\left[(\varepsilon_t +
...
-   Autocovariances : On en déduit pour tout $h$ dans $\N^*$ : \begin{eqnarray*}
    \gamma_X(h)&=&\sigma^2(\theta_h+\theta_1\theta_{1+h}+\cdots+\theta_{q-h}\theta_q)
    =\sigma^2\left(\theta_h+\sum_{i=1}^{q-h}\theta_i\theta_{i+h}\right).
    \end{eqnarray*}

::: proposition
**Autocorrélations simples d'un MA(q)** - Les autocorrélations simples d'un processus MA(q) sont nulles à partir du rang $q+1$ : $\rho_X(q)\neq 0$ et $\rho_X(h)=0$, pour $h>q$. - La propriété d'autocorrélations simples nulles à partir d'un certain rang $q+1$ est caractéristique d'un MA($q$)
:::
...
ggacf(Xt1)+ggtitle(TeX(r'($X_t = \epsilon_t - 0.7 \epsilon_{t-1}+0.1 \epsilon_{t-2}$)'))+
theme(plot.title = element_text(size = 40))
```
...
ggacf(Xt2)+ggtitle(TeX(r'($X_t = \epsilon_t - 0.7 \epsilon_{t-1}+0.1 \epsilon_{t-2} -0.2 \epsilon_{t-3}$)'))+
theme(plot.title = element_text(size = 40))
```
:::

::: proposition
**Autocorrélations partielles d'un MA(q)** Les autocorrélations partielles d'un processus MA(q) sont solutions d'une équation linéaire récurrente d'ordre $q$. Elles décroissent, de manière exponentielle ou sinusoïdale amortie, vers 0.
:::
...
MA(2) - racines $2\pm i 2\sqrt{3}/3$

$X_t = \varepsilon_t + 0.4 \varepsilon_{t-1} - 0.45 \varepsilon_{t-2}$
...
::: {#def-ARMA .definition}
On dit qu'un processus stationnaire $(X_t)_{t\in \Z}$ admet une représentation **ARMA**$(p,q)$ s'il vérifie l'équation $$ 
...
::: definition
En utilisant les polynômes en $B$ \begin{eqnarray*} \Phi(B)&=&I-\varphi_1B-\varphi_2B^2-\cdots-\varphi_pB^p\\
\text{ et }\Theta(B)&=&I+\theta_1B+\theta_2B^2+\cdots+\theta_qB^q,
\end{eqnarray*} un processus ARMA(p,q) vérifie l'équation : $$
\Phi(B)X_t=\Theta(B)\varepsilon_t,\ \forall t\in\Z. 
$$
:::
...
-   Les processus ARMA sont très importants en pratique car on peut montrer que tout processus stationnaire peut être approché par un processus ARMA.
...
Si la représentation n'est pas minimale ($\Phi(z)$ et $\Theta(z)$ ont une ou des racine(s) commune(s)), alors deux cas peuvent se présenter :
...
La représentation étant causale, les racines du polynôme $\Phi(z)$ sont toutes à l'extérieur du disque unité et l'inverse de $\Phi(z)$ s'écrit alors comme une série de puissances positives de $B$. Multipliée par $\Theta(B)$, on garde une série en puissances positives de $B$ et on a donc l'écriture : $$
X_t=\Phi^{-1}(B)\Theta(B)\varepsilon_t=\varepsilon_t+\sum_{i=1}^{+\infty}\psi_i\varepsilon_{t-i},
...
&=\varepsilon_t
+\theta_1\varepsilon_{t-1}+\theta_2\varepsilon_{t-2}+\cdots+\theta_q\varepsilon_{t-q}.
\end{eqnarray*} En identifiant les coefficients devant les $\varepsilon_t$, on obtient les équations : \begin{eqnarray*} &&\psi_1-\varphi_1=\theta_1,\\
&&\psi_2-\varphi_1\psi_1-\varphi_2=\theta_2,\\
&&\psi_3-\varphi_1\psi_2-\varphi_2\psi_1-\varphi_3=\theta_3\\ \end{eqnarray*} et ainsi de suite. C'est bien la formule annoncée dans la proposition.
:::
...
Autocorrélation avec l'écriture ARMA($p,q$)

-   Soit $X_{t+h}-\underset{j=1}{\stackrel{p}{\sum}}\varphi_jX_{t+h-j}=\varepsilon_{t+h} +\underset{i=1}{\stackrel{q}{\sum}}\theta_i\varepsilon_{t+h-i}$
-   La covariance de chaque terme avec $X_t$ donne \scriptsize  \begin{eqnarray*}
    ...
    \end{eqnarray*}
-   Pour $h>q$, on a $\gamma_X(h)-\underset{j=1}{\stackrel{p}{\sum}}\varphi_j\gamma_X(h-j)=0.$
...
Autocorrélation avec l'écriture MA($\infty$)
...
-   On constate ainsi, qu'à la différence des cas particuliers des processus AR($p$) ou MA($q$), il n'existe pas de caractérisation aisée pour les modèles ARMA($p,q$). Les autocorrélations simples ou partielles ne s'annulent pas à partir d'un certain rang.
...
-   Dans la pratique les processus sont rarement stationnaires

-   Généralisation des processus ARMA pour les étendre à des processus non stationnaires.

-   L'idée générale, essentiellement conçue pour les processus non stationnaires à tendance polynomiale, est de différencier suffisamment le processus initial afin d'obtenir un processus sans tendance et appliquer un modèle ARMA sur le processus différencié. Cette classe de modèle est connue sous le nom de processus **ARIMA (AutoRegressive Integrated Moving Average)**.

::: {#def-arima .definition}
On dit qu'un processus $(X_t)_{t\in \Z}$ admet une représentation **ARIMA**$(p,d,q)$ ($p,d,q\in\mathbb N$) s'il vérifie l'équation : $$
...
g1<-autoplot(acf(diff(ts.sim),plot=F))+ggtitle("ARIMA(3,1,0)")
g2<-autoplot(pacf(diff(ts.sim),plot=F))+ggtitle("ARIMA(3,1,0)")
...
g1<-autoplot(acf(diff(ts.sim),plot=F))+ggtitle("ARIMA(0,1,3)")
g2<-autoplot(pacf(diff(ts.sim),plot=F))+ggtitle("ARIMA(0,1,3)")
grid.arrange(g1,g2,ncol=2)
```

### Sélection de modèle

Si on a plusieurs modèles ARIMA en compétition $\texttt{mod}_1,\ldots,\texttt{mod}_K$

-   On estime les paramètres de chaque modèle $\texttt{mod}_k$ considéré (par maximum de vraisemblance le plus souvent)

-   On utilise un critère pénalisé pour choisir le meilleur modèle

    -   $\mbox{AIC}(\texttt{mod}_k) = -2 \mbox{ log. vrais.} + 2 (p+q+1+\mathbb{1}_{c\neq 0})$

    -   $\mbox{AICc}(\texttt{mod}_k) = \mbox{AIC}(\texttt{mod}_k) + \frac{2 (p+q+1+\mathbb{1}_{c\neq 0})(p+q+2+\mathbb{1}_{c\neq 0})}{n-p-q-2-\mathbb{1}_{c\neq 0}}$

    -   $\mbox{BIC}(\texttt{mod}_k) = -2 \mbox{ log. vrais.} + (p+q+1+\mathbb{1}_{c\neq 0}) \log(n)$

::: { .example #exm-selmod}

-   Données simulées sous un modèle ARIMA(1,1,2) $$
...
SARIMA = Seasonal ARIMA : extension des modèles ARIMA à des séries présentant une saisonnalité.

::: definition
On dit qu'un processus $(X_t)_{t\in \Z}$ admet une représentation **SARIMA**$(p,d,q)(P,D,Q)[s]$ s'il vérifie l'équation : $$
...
::: {#exm-sarima .example}
-   Données simulées selon un modèle SARIMA(0,1,1)(0,1,1)\[12\] $$
...
g1<-autoplot(Xt)
g2<-gglagplot(Xt,lags=12,do.lines=F)
grid.arrange(g1,g2,ncol=2)
```
...
#| echo: false
#| fig.height: 4
g1<-autoplot(acf(Xt,lag.max=60,plot=F))
g2<-autoplot(pacf(Xt,lag.max=60,plot=F))
grid.arrange(g1,g2,ncol=2)
```
...
#| fig-height: 4
Yt<-diff(diff(Xt),12)
g1<-autoplot(acf(Yt,lag.max=50,plot=F))
g2<-autoplot(pacf(Yt,lag.max=50,plot=F))+ylab("pACF")
grid.arrange(g1,g2,ncol=2)
```
...
-   ACF / pACF des données différenciées

```{r}
#| echo: false
#| fig-height: 3.5
g1<-autoplot(acf(diff(tta,24),plot=F,lag.max=100))
g2<-autoplot(pacf(diff(tta,24),plot=F,lag.max = 100))
grid.arrange(g1,g2,ncol=2)
```

$\Longrightarrow$ on peut considérer des modèles ARIMA avec

-   $0\leq q\leq 12$, $0\leq p \leq 2$
